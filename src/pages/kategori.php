...
if ($action === 'add_kategori' && $_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   header("Location: /inventaris-barang-kantor/kategori");
    exit;
}
...
// DELETE
if ($action === 'delete_kategori' && isset($_POST['id'])) {
    ...
    try {
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $_SESSION['message'] = 'Kategori tidak ditemukan atau gagal dihapus.';
            $_SESSION['message_type'] = 'error';
        }
        $stmt->close();
    } catch (\Exception $e) {
...
try {
    ...
    if ($result) {
        ...
        $_SESSION['message'] = 'Gagal mengambil data kategori: ' . $conn->error;
        $_SESSION['message_type'] = 'error';
    }
} finally {
    $db->close();
...
<body class="bg-blue-50">
    ...
    <div class="flex h-screen">
                                ...
                                <?php foreach ($categories as $category): ?>
                                    <tr class="bg-white border-b hover:bg-blue-50">
                                        <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                            <?php echo htmlspecialchars($category['id_kategori']); ?></td>
                                        ...
                                        <td class="px-6 py-4">
                                            ...
                                            <form action="/inventaris-barang-kantor/kategori" method="POST" style="display:inline;">
        ...
        <button type="submit" class="font-medium text-red-600 hover:underline bg-transparent border-0 p-0 m-0 cursor-pointer">Hapus</button>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="3" class="px-6 py-4 text-center text-gray-500">Belum ada kategori atau
                                        gagal memuat data.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
    ...
    <div id="defaultModal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-2xl md:h-auto">
            <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
                ...
                <form action="/inventaris-barang-kantor/kategori" method="POST">
                    <input type="hidden" name="action" value="add_kategori">
                    <div class="grid gap-4 mb-4">
                        <div>
                            <label for="nama_kategori_add" class="block mb-2 text-sm font-medium text-gray-900">Nama
                                Kategori</label>
                            <input type="text" name="nama_kategori" id="nama_kategori_add"
        ...
        <div class="relative p-4 w-full max-w-2xl md:h-auto">
            <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
                ...
                <form action="/inventaris-barang-kantor/kategori" method="POST">
                    ...
                    <div class="grid gap-4 mb-4">
                        <div>
                            <label for="nama_kategori_edit" class="block mb-2 text-sm font-medium text-gray-900">Nama
                                Kategori</label>
                            <input type="text" name="nama_kategori_edit" id="nama_kategori_edit"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                                required>
                        </div>
                    </div>
