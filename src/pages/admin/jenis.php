...
<body>
  <!-- Sidebar (copy dari kategori.php, pastikan menu Jenis aktif) -->
  ...
  <div id="mainContent" class="min-h-screen flex flex-col transition-all duration-300 sm:ml-64 main-content">
    <header class="bg-white shadow px-6 py-4 flex justify-between items-center">
      <h1 class="text-2xl font-bold text-gray-800">Manajemen Jenis</h1>
      <div class="flex gap-2">
        <form method="GET" action="<?= Helper::basePath(); ?>jenis" class="flex gap-2">
          <select name="sort" class="border rounded px-2 py-1 text-sm no-arrow">
          ...
          <button type="submit" class="bg-gray-200 hover:bg-gray-300 px-3 py-1 rounded text-sm font-semibold">Urutkan</button>
        </form>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <?php foreach ($jenis as $j): ?>
          <div class="bg-white rounded-xl shadow p-6 flex flex-col justify-between">
            <div>
              <div class="text-sm text-gray-400 mb-1">ID: <?= htmlspecialchars($j['id_jenis']) ?></div>
              <div class="text-xl font-semibold text-gray-800 mb-2"><?= htmlspecialchars($j['nama']) ?></div>
              <div class="mb-2">
  ...
  <div id="modalTambah" tabindex="-1" class="hidden fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto h-modal h-full bg-black/40">
    <div class="relative w-full max-w-md mx-auto mt-24">
      <div class="bg-white rounded-lg shadow p-6">
        <h3 class="text-lg font-bold mb-4">Tambah Jenis</h3>
        <form method="POST" action="<?= Helper::basePath(); ?>jenis">
          <input type="hidden" name="action" value="add_jenis">
          <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">ID Jenis</label>
            <input type="text" name="id_jenis" class="w-full border rounded px-3 py-2" maxlength="10" required>
          </div>
          <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">Nama Jenis</label>
            <input type="text" name="nama_jenis" class="w-full border rounded px-3 py-2" required>
          </div>
          <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">Stok</label>
            <input type="number" name="stok" class="w-full border rounded px-3 py-2" min="0" value="0">
          </div>
          <div class="mb-4">
  ...
  <div id="modalEdit" tabindex="-1" class="hidden fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto h-modal h-full bg-black/40">
    <div class="relative w-full max-w-md mx-auto mt-24">
      <div class="bg-white rounded-lg shadow p-6">
        <h3 class="text-lg font-bold mb-4">Edit Jenis</h3>
        <form method="POST" action="<?= Helper::basePath(); ?>jenis">
          <input type="hidden" name="action" value="edit_jenis">
          <input type="hidden" name="id_jenis" id="edit_id_jenis">
          <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">Nama Jenis</label>
            <input type="text" name="nama_jenis" id="edit_nama_jenis" class="w-full border rounded px-3 py-2" required>
          </div>
          <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">Stok</label>
            <input type="number" name="stok" id="edit_stok" class="w-full border rounded px-3 py-2" min="0">
          </div>
          <div class="mb-4">
            <label class="block mb-1 font-medium text-gray-700">Kategori</label>
            <select name="kategori_id" id="edit_kategori_id" class="w-full border rounded px-3 py-2" required>
              <option value="">Pilih Kategori</option>
              <?php foreach ($kategori as $k): ?>
                <option value="<?= $k['id_kategori'] ?>"><?= htmlspecialchars($k['nama']) ?></option>
              <?php endforeach; ?>
